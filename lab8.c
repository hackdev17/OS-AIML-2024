...
Directory singleLevelDirectory,twoLevelDirectory[MAX_FILES];
...
void displaySingleLevelDirectory(){
	printf("Single Level Directory :\nDirectory Name : %s\nNumber of Files : %d\n", \
		singleLevelDirectory.name,singleLevelDirectory.numFiles \
	);
...
void displayTwoLevelDirectory(){
	...
	for(int i=0;i<MAX_FILES;++i){
		printf( \
			"Directory Name : %s\nNumber of Files : %d\nFiles :\n", \
			twoLevelDirectory[i].name,twoLevelDirectory[i].numFiles
		);
